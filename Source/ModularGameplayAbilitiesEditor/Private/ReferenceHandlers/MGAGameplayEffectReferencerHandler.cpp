...
#define LOCTEXT_NAMESPACE "MGAGameplayEffectReferencerHandler"

TSharedPtr<IMGAAttributeReferencerHandler> FMGAGameplayEffectReferencerHandler::Create()
{
	return MakeShared<FMGAGameplayEffectReferencerHandler>();
}

FMGAGameplayEffectReferencerHandler::~FMGAGameplayEffectReferencerHandler()
{
	AttributesCacheMap.Reset();
}

void FMGAGameplayEffectReferencerHandler::OnPreCompile(const FString& InPackageName)
{
	AttributesCacheMap.Reset();
}

void FMGAGameplayEffectReferencerHandler::OnPostCompile(const FString& InPackageName)
{
}

bool FMGAGameplayEffectReferencerHandler::HandlePreCompile(const FAssetIdentifier& InAssetIdentifier, const FMGAAttributeReferencerPayload& InPayload)
{
	MGA_EDITOR_NS_LOG(Verbose, TEXT("InAssetIdentifier: %s, InPayload: %s"), *InAssetIdentifier.ToString(), *InPayload.ToString())
...
bool FMGAGameplayEffectReferencerHandler::HandleAttributeRename(const FAssetIdentifier& InAssetIdentifier, const FMGAAttributeReferencerPayload& InPayload, TArray<TSharedRef<FTokenizedMessage>>& OutMessages)
{
	MGA_EDITOR_NS_LOG(Verbose, TEXT("InAssetIdentifier: %s, InPayload: %s"), *InAssetIdentifier.ToString(), *InPayload.ToString())
	...
	if (!Blueprint)
	{
		MGA_EDITOR_NS_LOG(Warning, TEXT("Failed to update modifiers because of invalid Blueprint for %s"), *InPayload.PackageName)
		return false;
	}
...
bool FMGAGameplayEffectReferencerHandler::BuildModifierInfoAttributeReference(const FGameplayModifierInfo& InModifier, FAttributeReference& OutAttributeReference)
{
...
bool FMGAGameplayEffectReferencerHandler::BuildModifierMagnitudeAttributeReference(const FGameplayEffectModifierMagnitude& InMagnitude, FAttributeReference& OutAttributeReference)
{
...
bool FMGAGameplayEffectReferencerHandler::BuildEffectCueMagnitudeAttributeReference(const FGameplayEffectCue& InEffectCue, FAttributeReference& OutAttributeReference)
{
	FString FinalValue;
	...
	FAttributeReference AttributeReference;
	if (!UMGAEditorSubsystem::ParseAttributeFromDefaultValue(FinalValue, AttributeReference.PackageNameOwner, AttributeReference.AttributeName))
	{
		return false;
	}
...
bool FMGAGameplayEffectReferencerHandler::BuildEffectQueryAttributeReference(const FGameplayEffectQuery& InEffectQuery, FAttributeReference& OutAttributeReference)
{
	FString FinalValue;
...
bool FMGAGameplayEffectReferencerHandler::UpdateModifiers(UGameplayEffect* InEffectCDO, const UBlueprint* InBlueprint, const FAssetIdentifier& InAssetIdentifier, const FMGAAttributeReferencerPayload& InPayload, TArray<TSharedRef<FTokenizedMessage>>& OutMessages)
{
	MGA_EDITOR_NS_LOG(Display, TEXT("InEffectCDO: %s, InBlueprint: %s, InOldPropertyName: %s, InNewPropertyName: %s"),
	...
	MGA_EDITOR_LOG(VeryVerbose, TEXT("UMGAEditorSubsystem::UpdateGameplayEffectModifiers Blueprint: %s"), *GetNameSafe(InBlueprint))
	int32 CurrentIndex = 0;
	...
	MGA_EDITOR_LOG(Verbose, TEXT("UMGAEditorSubsystem::UpdateGameplayEffectModifiers Update CDO modifiers now from gathered props to replace: %d"), ModifiersToReplace.Num())
	for (const FAttributeModifierToReplace& ModifierToReplace : ModifiersToReplace)
	{
		...
		{
			MGA_EDITOR_LOG(Verbose, TEXT("Invalid index %d for CDO modifiers"), Index)
			continue;
		}
		...
		if (InPayload.ReferencerBlueprint.IsValid())
		{
			Message->AddToken(
				...
				->OnMessageTokenActivated(FOnMessageTokenActivated::CreateStatic(&UMGAEditorSubsystem::HandleMessageLogLinkActivated))
			);
		}
...
bool FMGAGameplayEffectReferencerHandler::UpdateModifiersBackingAttribute(UGameplayEffect* InEffectCDO, const UBlueprint* InBlueprint, const FAssetIdentifier& InAssetIdentifier, const FMGAAttributeReferencerPayload& InPayload, TArray<TSharedRef<FTokenizedMessage>>& OutMessages)
{
	MGA_EDITOR_NS_LOG(Display, TEXT("InEffectCDO: %s, InBlueprint: %s, InOldPropertyName: %s, InNewPropertyName: %s"),
		*GetNameSafe(InEffectCDO),
		*GetNameSafe(InBlueprint),
	...
	for (FGameplayModifierInfo& Modifier : InEffectCDO->Modifiers)
	{
		...
		if (CalculationType == EGameplayEffectMagnitudeCalculation::AttributeBased && Definitions.IsValidIndex(0))
		{
			...
			if (CachedAttribute.PackageNameOwner == InPayload.PackageName && CachedAttribute.AttributeName == InPayload.OldPropertyName)
			{
				...
				MGA_EDITOR_NS_LOG(VeryVerbose, TEXT("Replacing modifier magnitude backing attribute with %s"), *GetNameSafe(NewAttributeProp))
...
bool FMGAGameplayEffectReferencerHandler::UpdateDuration(UGameplayEffect* InEffectCDO, const UBlueprint* InBlueprint, const FAssetIdentifier& InAssetIdentifier, const FMGAAttributeReferencerPayload& InPayload, TArray<TSharedRef<FTokenizedMessage>>& OutMessages)
{
	MGA_EDITOR_NS_LOG(
		Verbose,
	...
	if (DurationMagnitude.GetMagnitudeCalculationType() == EGameplayEffectMagnitudeCalculation::AttributeBased && Definitions.IsValidIndex(0))
	{
		...
		if (CachedAttribute.PackageNameOwner == InPayload.PackageName && CachedAttribute.AttributeName == InPayload.OldPropertyName)
		{
			...
			MGA_EDITOR_NS_LOG(VeryVerbose, TEXT("Replacing duration magnitude attribute with %s"), *GetNameSafe(NewAttributeProp))
...
bool FMGAGameplayEffectReferencerHandler::UpdateCues(UGameplayEffect* InEffectCDO, const UBlueprint* InBlueprint, const FAssetIdentifier& InAssetIdentifier, const FMGAAttributeReferencerPayload& InPayload, TArray<TSharedRef<FTokenizedMessage>>& OutMessages)
{
	MGA_EDITOR_NS_LOG(
		Verbose,
...
bool FMGAGameplayEffectReferencerHandler::UpdateRemoveGameplayEffectQuery(UGameplayEffect* InEffectCDO, const UBlueprint* InBlueprint, const FAssetIdentifier& InAssetIdentifier, const FMGAAttributeReferencerPayload& InPayload, TArray<TSharedRef<FTokenizedMessage>>& OutMessages)
{
#if UE_VERSION_NEWER_THAN(5, 3, -1)
...
bool FMGAGameplayEffectReferencerHandler::UpdateImmunityEffectQuery(UGameplayEffect* InEffectCDO, const UBlueprint* InBlueprint, const FAssetIdentifier& InAssetIdentifier, const FMGAAttributeReferencerPayload& InPayload, TArray<TSharedRef<FTokenizedMessage>>& OutMessages)
{
#if UE_VERSION_NEWER_THAN(5, 3, -1)
...
bool FMGAGameplayEffectReferencerHandler::UpdateEffectQuery(const UBlueprint* InBlueprint, const FAssetIdentifier& InAssetIdentifier, EAttributeReferenceType InReferenceType, const FMGAAttributeReferencerPayload& InPayload, const FText& InMessageText, FGameplayAttribute& OutModifyingAttribute, TArray<TSharedRef<FTokenizedMessage>>& OutMessages)
{
	MGA_EDITOR_NS_LOG(Verbose, TEXT("InBlueprint: %s, InAssetIdentifier: %s, InPayload: %s"), *GetNameSafe(InBlueprint), *InAssetIdentifier.ToString(), *InPayload.ToString())
	...
	if (CachedAttribute.PackageNameOwner == InPayload.PackageName && CachedAttribute.AttributeName == InPayload.OldPropertyName)
	{
		...
		MGA_EDITOR_NS_LOG(VeryVerbose, TEXT("Replacing %s with %s"), *InMessageText.ToString(), *GetNameSafe(NewAttributeProp))
...
FAttributeBasedFloat* FMGAGameplayEffectReferencerHandler::GetAttributeBasedFloatFromContainer(FGameplayEffectModifierMagnitude* InModifierMagnitude) const
{
...
FString FMGAGameplayEffectReferencerHandler::GetClassDefaultName(const FString& InName)
{
	FString Name = InName;
