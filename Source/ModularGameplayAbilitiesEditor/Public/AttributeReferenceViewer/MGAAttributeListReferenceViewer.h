...
struct FMGAAttributeListReferenceViewerNode
{
	FMGAAttributeListReferenceViewerNode(FProperty* InAttribute, const FString& InAttributeName)
	{
...
class MODULARGAMEPLAYABILITIESEDITOR_API SMGAAttributeListReferenceViewer : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SMGAAttributeListReferenceViewer)
...
private:
	...
	TSharedPtr<SListView<TSharedPtr<FMGAAttributeListReferenceViewerNode>>> AttributeList;
	
	/** Array of items that can be selected in the dropdown menu */
	TArray<TSharedPtr<FMGAAttributeListReferenceViewerNode>> PropertyOptions;
	...
	TSharedRef<ITableRow> OnGenerateRowForAttributeViewer(TSharedPtr<FMGAAttributeListReferenceViewerNode> InItem, const TSharedRef<STableViewBase>& InOwnerTable) const;
	
	/** Called when an item is selected from the list. */
	void OnAttributeSelectionChanged(TSharedPtr<FMGAAttributeListReferenceViewerNode> InItem, ESelectInfo::Type SelectInfo) const;
};
