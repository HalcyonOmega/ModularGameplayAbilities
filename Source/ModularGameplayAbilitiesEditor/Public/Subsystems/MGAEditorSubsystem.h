...
#include "MGAEditorSubsystem.generated.h"

class FTokenizedMessage;
class IMGAAttributeReferencerHandler;
class IMessageToken;
class IToolkit;
class UEdGraphPin;
 ...
 * ---
 *
 * Subsystem register to an OnVariableRename delegate fired off from `UMGAAttributeSetBlueprint`, and handle any editor specific code.
 *
 ...
 */
UCLASS()
class MODULARGAMEPLAYABILITIESEDITOR_API UMGAEditorSubsystem : public UEditorSubsystem
{
	GENERATED_BODY()
...
public:
	...
	static UMGAEditorSubsystem& Get();
	...
	virtual void RegisterReferencerHandler(const FName& InClassName, const TSharedPtr<IMGAAttributeReferencerHandler>& InHandler);
	...
	TSharedPtr<IMGAAttributeReferencerHandler> FindAssetDependencyHandler(const FName& InReferencer, TWeakObjectPtr<UObject>& OutDefaultObject);
	...
	/**
	 * Triggered by FMGABlueprintEditor just before FBlueprintEditor::Compile()
	 *
