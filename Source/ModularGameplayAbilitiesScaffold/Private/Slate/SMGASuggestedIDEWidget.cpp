...
#define LOCTEXT_NAMESPACE "SMGASuggestedIDEWidget"

TSharedPtr<SNotificationItem> SMGASuggestedIDEWidget::IDEDownloadNotification;

void SMGASuggestedIDEWidget::Construct(const FArguments& InArgs)
{
	TAttribute<EVisibility> DefaultVisibility;
	DefaultVisibility.BindStatic(&SMGASuggestedIDEWidget::GetVisibility);
...
TSharedRef<SWidget> SMGASuggestedIDEWidget::CreateGetSuggestedIDEWidget()
{
	...
	if (FSourceCodeNavigation::GetCanDirectlyInstallSourceCodeIDE())
	{
		...
		return			
			...
			.OnClicked(this, &SMGASuggestedIDEWidget::OnInstallIDEClicked);
	}
	
	return	
		SNew(SPrimaryButton)
		.Text(FText::Format(LOCTEXT("IDEDownloadLinkText", "Download {0}"), FSourceCodeNavigation::GetSuggestedSourceCodeIDE()))
		.OnClicked_Static(&SMGASuggestedIDEWidget::OnDownloadIDEClicked, FSourceCodeNavigation::GetSuggestedSourceCodeIDEDownloadURL());
}

TSharedRef<SWidget> SMGASuggestedIDEWidget::CreateGetDocumentationLinkWidget() const
{
	return	
		SNew(SPrimaryButton)
		.Text(FText::Format(LOCTEXT("IDESetupText", "Setting Up {0}"), FSourceCodeNavigation::GetSuggestedSourceCodeIDE()))
		.OnClicked_Static(&SMGASuggestedIDEWidget::OnRedirectToDocumentation);
}

EVisibility SMGASuggestedIDEWidget::GetVisibility()
{
	return FSourceCodeNavigation::IsCompilerAvailable() ? EVisibility::Collapsed : EVisibility::Visible;
}

FReply SMGASuggestedIDEWidget::OnDownloadIDEClicked(const FString InURL)
{
	FPlatformProcess::LaunchURL(*InURL, nullptr, nullptr);
	return FReply::Handled();
}

FReply SMGASuggestedIDEWidget::OnRedirectToDocumentation()
{
	FPlatformProcess::LaunchURL(DocumentationURL, nullptr, nullptr);
	return FReply::Handled();
}

FReply SMGASuggestedIDEWidget::OnInstallIDEClicked()
{
	...
	if (!IDEDownloadNotification.IsValid() || IDEDownloadNotification->GetCompletionState() != SNotificationItem::ECompletionState::CS_Pending)
	{
		...
		FSourceCodeNavigation::DownloadAndInstallSuggestedIDE(FOnIDEInstallerDownloadComplete::CreateStatic(&SMGASuggestedIDEWidget::OnIDEInstallerDownloadComplete));
...
void SMGASuggestedIDEWidget::OnIDEInstallerDownloadComplete(const bool bWasSuccessful)
{
	if (IDEDownloadNotification.IsValid())
	{
		if (bWasSuccessful)
