...
#define LOCTEXT_NAMESPACE "SMGABlueprintHeaderView"

FString SMGAClassInfo::LastSelectedModuleName = TEXT("");

BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION

// ReSharper disable once CppParameterNeverUsed
void SMGAClassInfo::Construct(const FArguments& InArgs, const TSharedPtr<FMGAAttributeSetWizardViewModel>& InViewModel)
{
				...
				[
					...
					[
						...
						[
							...
							.OnValueChanged(this, &SMGAClassInfo::OnClassLocationChanged)
							.Value(this, &SMGAClassInfo::GetClassLocation)
						...
						[
							...
							[
								...
								.OnGetMenuContent(this, &SMGAClassInfo::GetClassPickerMenuContent)
								.ButtonContent()
								[
									SNew(STextBlock)
									.Text(this, &SMGAClassInfo::GetClassPickerText, false)
									.ToolTipText(this, &SMGAClassInfo::GetClassPickerText, true)
								]
							]
						]
					]
					...
					[
						...
						[
							...
							[
								...
								.Text(this, &SMGAClassInfo::OnGetClassNameText)
								.OnTextChanged(this, &SMGAClassInfo::OnClassNameTextChanged)
								.ToolTipText(LOCTEXT("NameDescription", "This is the name of the class and header / source filenames"))
							]
							...
							[
								...
								.OnSelectionChanged(this, &SMGAClassInfo::HandleSelectedModuleComboBoxSelectionChanged)
								.OnGenerateWidget(this, &SMGAClassInfo::MakeWidgetForSelectedModuleCombo)
								[
									SNew(STextBlock)
									.Text(this, &SMGAClassInfo::GetSelectedModuleComboText)
								]
							]
						]
					]
						...
						[
							...
							[
								...
								[
									...
									.Text(this, &SMGAClassInfo::OnGetClassPathText)
									.OnTextChanged(this, &SMGAClassInfo::OnClassPathTextChanged)
									.IsEnabled(this, &SMGAClassInfo::IsClassPathTextEnabled)
									.ToolTipText(LOCTEXT("NameDescription", "This is the relative path beneath the target module source path"))
								]
								...
								[
									...
									.OnClicked(this, &SMGAClassInfo::HandleChooseFolderButtonClicked)
									[
										SNew(SImage)
					...
					[
						...
						[
							SNew(STextBlock)
							.Text(this, &SMGAClassInfo::OnGetClassHeaderFileText)
						]
					]
					...
					[
						...
						[
							SNew(STextBlock)
							.Text(this, &SMGAClassInfo::OnGetClassSourceFileText)
						]
					]
				]
			]
...
void SMGAClassInfo::InitAvailableModules()
{
...
TSharedPtr<FModuleContextInfo> SMGAClassInfo::InitSelectedModuleInfo()
{
...
GameProjectUtils::EClassLocation SMGAClassInfo::GetClassLocation() const
{
	check(ViewModel.IsValid());
	return ViewModel->GetClassLocation();
}

void SMGAClassInfo::OnClassLocationChanged(const GameProjectUtils::EClassLocation InLocation) const
{
	check(ViewModel.IsValid());
...
FText SMGAClassInfo::OnGetClassNameText() const
{
	check(ViewModel.IsValid());
	return FText::FromString(ViewModel->GetNewClassName());
}

void SMGAClassInfo::OnClassNameTextChanged(const FText& InNewText) const
{
	check(ViewModel.IsValid());
...
FText SMGAClassInfo::GetSelectedModuleComboText() const
{
	check(ViewModel.IsValid());
...
// ReSharper disable once CppParameterNeverUsed
void SMGAClassInfo::HandleSelectedModuleComboBoxSelectionChanged(const TSharedPtr<FModuleContextInfo> InSelectedModuleInfo, ESelectInfo::Type InSelectInfo) const
{
	check(ViewModel.IsValid());
...
TSharedRef<SWidget> SMGAClassInfo::MakeWidgetForSelectedModuleCombo(const TSharedPtr<FModuleContextInfo> InValue) const
{
	FFormatNamedArguments Args;
...
FText SMGAClassInfo::OnGetClassPathText() const
{
	check(ViewModel.IsValid());
	return FText::FromString(ViewModel->GetSelectedClassPath());
}

bool SMGAClassInfo::IsClassPathTextEnabled() const
{
	check(ViewModel.IsValid());
...
void SMGAClassInfo::OnClassPathTextChanged(const FText& InNewText)
{
	check(ViewModel.IsValid());
...
FString SMGAClassInfo::CalculateNewClassPath(const GameProjectUtils::EClassLocation InLocation, const FString& InNewClassPath) const
{
	check(ViewModel.IsValid());
...
FText SMGAClassInfo::OnGetClassHeaderFileText() const
{
	check(ViewModel.IsValid());
	return FText::FromString(ViewModel->GetCalculatedClassHeaderName());
}

FText SMGAClassInfo::OnGetClassSourceFileText() const
{
	check(ViewModel.IsValid());
	return FText::FromString(ViewModel->GetCalculatedClassSourceName());
}

FReply SMGAClassInfo::HandleChooseFolderButtonClicked()
{
...
FText SMGAClassInfo::GetClassPickerText(const bool bIsTooltipText) const
{
	check(ViewModel.IsValid());
...
class FMGABlueprintParentFilter : public IClassViewerFilter
{
public:
	/** All children of these classes will be included unless filtered out by another setting. */
	TSet<const UClass*> AllowedChildrenOfClasses;

	explicit FMGABlueprintParentFilter(const TSet<const UClass*>& InAllowedChildrenOfClasses)
		: AllowedChildrenOfClasses(InAllowedChildrenOfClasses)
	{
	}

	explicit FMGABlueprintParentFilter(const UClass* InAllowedClass)
	{
...
TSharedRef<SWidget> SMGAClassInfo::GetClassPickerMenuContent()
{
	FClassViewerModule& ClassViewerModule = FModuleManager::LoadModuleChecked<FClassViewerModule>("ClassViewer");

	FClassViewerInitializationOptions Options;

	Options.ClassFilters.Add(MakeShared<FMGABlueprintParentFilter>(UAttributeSet::StaticClass()));
	...
	return SNew(SBox)
		...
		[
			ClassViewerModule.CreateClassViewer(
				Options,
				FOnClassPicked::CreateSP(this, &SMGAClassInfo::OnParentClassSelected)
			)
		];
}

void SMGAClassInfo::OnParentClassSelected(UClass* InParentClass) const
{
	check(ViewModel.IsValid());
