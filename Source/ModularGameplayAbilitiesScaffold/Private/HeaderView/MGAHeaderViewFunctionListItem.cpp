...
#define LOCTEXT_NAMESPACE "FMGAHeaderViewFunctionListItem"

FMGAHeaderViewListItemPtr FMGAHeaderViewFunctionListItem::Create(const UK2Node_FunctionEntry* FunctionEntry)
{
	return MakeShareable(new FMGAHeaderViewFunctionListItem(FunctionEntry));
}

void FMGAHeaderViewFunctionListItem::ExtendContextMenu(FMenuBuilder& InMenuBuilder, const TWeakObjectPtr<UObject> InAsset)
{
	if (UBlueprint* Blueprint = Cast<UBlueprint>(InAsset.Get()))
	{
		...
		InMenuBuilder.AddMenuEntry(LOCTEXT("JumpToDefinition", "Jump to Definition"),
			...
			FSlateIcon(),
			FUIAction(FExecuteAction::CreateSP(this, &FMGAHeaderViewFunctionListItem::JumpToDefinition, WeakBlueprint))
		);
		...
		if (!IllegalName.IsNone())
		{
			InMenuBuilder.AddVerifiedEditableText(LOCTEXT("RenameItem", "Rename Function"),
				...
				FOnVerifyTextChanged::CreateStatic(&FMGAHeaderViewFunctionListItem::OnVerifyRenameFunctionTextChanged, WeakBlueprint),
				FOnTextCommitted::CreateStatic(&FMGAHeaderViewFunctionListItem::OnRenameFunctionTextCommitted, WeakBlueprint, GraphName)
			);
		}
		...
		for (FName IllegalParam : IllegalParameters)
		{
			InMenuBuilder.AddVerifiedEditableText(LOCTEXT("RenameParm", "Rename Parameter"),
				...
				FOnVerifyTextChanged::CreateSP(this, &FMGAHeaderViewFunctionListItem::OnVerifyRenameParameterTextChanged, WeakBlueprint, GraphName),
				FOnTextCommitted::CreateSP(this, &FMGAHeaderViewFunctionListItem::OnRenameParameterTextCommitted, WeakBlueprint, GraphName, IllegalParam)
			);
		}
#endif
	}
...
void FMGAHeaderViewFunctionListItem::OnMouseButtonDoubleClick(const TWeakObjectPtr<UObject> InAsset)
{
	const TWeakObjectPtr<UBlueprint> WeakBlueprint = Cast<UBlueprint>(InAsset.Get());
	JumpToDefinition(WeakBlueprint);
}

FString FMGAHeaderViewFunctionListItem::GetConditionalUFunctionSpecifiers(const UFunction* SigFunction)
{
	check(SigFunction);
...
void FMGAHeaderViewFunctionListItem::AppendFunctionParameters(const UFunction* SignatureFunction)
{
	...
	for (TFieldIterator<const FProperty> ParamIt(SignatureFunction); ParamIt; ++ParamIt)
	{
		...
		if (ParamIt->HasAnyPropertyFlags(CPF_ConstParm))
		{
			...
			RichTextString.Append(FString::Printf(TEXT("<%s>const</> "), *MGA::HeaderViewSyntaxDecorators::KeywordDecorator));
		}
		...
		else if (ParamIt->HasAnyPropertyFlags(CPF_ReferenceParm))
		{
			...
			RichTextString.Append(FString::Printf(TEXT("<%s>UPARAM</>(ref) "), *MGA::HeaderViewSyntaxDecorators::MacroDecorator));
		}
		...
		if (ParamIt->HasAnyPropertyFlags(CPF_OutParm | CPF_ReferenceParm))
		{
			...
			RichTextString.Append(FString::Printf(TEXT("<%s>%s</>& <%s>%s</>"), *MGA::HeaderViewSyntaxDecorators::TypenameDecorator, *Typename, **IdentifierDecorator, *ParamName));
		}
		else
		{
			RawItemString.Append(FString::Printf(TEXT("%s %s"), *Typename, *ParamName));
...
FMGAHeaderViewFunctionListItem::FMGAHeaderViewFunctionListItem(const UK2Node_FunctionEntry* FunctionEntry)
{
	check(FunctionEntry);
	...
	if (const UFunction* ResolvedFunc = FunctionEntry->FindSignatureFunction())
	{
		...
		if (ResolvedFunc->GetBoolMetaData(FBlueprintMetadata::MD_DeprecatedFunction))
		{
			...
			RichTextString += FString::Printf(TEXT("\n<%s>UE_DEPRECATED</>(%s, \"%s\")"), *MGA::HeaderViewSyntaxDecorators::MacroDecorator, *EngineVersionString, *DeprecationMessage);
		}
		...
		// i.e. UFUNCTION(BlueprintCallable, Category="Function Category")
		{
			...
			RichTextString += FString::Printf(TEXT("\n<%s>UFUNCTION</>(%s)"), *MGA::HeaderViewSyntaxDecorators::MacroDecorator, *AdditionalSpecifiers);
		}
		...
		// i.e. void FunctionName(Type InParam1, UPARAM(ref) Type2& InParam2, Type3& OutParam1)
		{
			...
			if (FProperty* ReturnProperty = ResolvedFunc->GetReturnProperty())
			{
				...
				RichTextString += FString::Printf(TEXT("\n<%s>%s</> <%s>%s</>("), *MGA::HeaderViewSyntaxDecorators::TypenameDecorator, *Typename, **IdentifierDecorator, *FunctionName);
			}
			else
			{
				RawItemString += FString::Printf(TEXT("\nvoid %s("), *FunctionName);
				RichTextString += FString::Printf(TEXT("\n<%s>void</> <%s>%s</>("), *MGA::HeaderViewSyntaxDecorators::KeywordDecorator, **IdentifierDecorator, *FunctionName);
			}
			...
			if (FunctionEntry->GetFunctionFlags() & FUNC_Const)
			{
				...
				RichTextString += FString::Printf(TEXT(") <%s>const</>;"), *MGA::HeaderViewSyntaxDecorators::KeywordDecorator);
			}
			else
			{
				RawItemString += TEXT(");");
...
void FMGAHeaderViewFunctionListItem::OnRenameFunctionTextCommitted(const FText& CommittedText, const ETextCommit::Type TextCommitType, const TWeakObjectPtr<UBlueprint> WeakBlueprint, FName OldGraphName)
{
	if (TextCommitType == ETextCommit::OnEnter)
	{
		if (UBlueprint* Blueprint = WeakBlueprint.Get())
		{
			...
			if (IsValidCPPIdentifier(CommittedString))
			{
				...
				{
					MGA_SCAFFOLD_LOG(Warning, TEXT("Could not find Function Graph named \"%s\" in Blueprint \"%s\""), *OldGraphName.ToString(), *Blueprint->GetName());
				}
			}
		}
	}
...
bool FMGAHeaderViewFunctionListItem::OnVerifyRenameFunctionTextChanged(const FText& InNewName, FText& OutErrorText, const TWeakObjectPtr<UBlueprint> WeakBlueprint)
{
...
bool FMGAHeaderViewFunctionListItem::OnVerifyRenameParameterTextChanged(const FText& InNewName, FText& OutErrorText, const TWeakObjectPtr<UBlueprint> WeakBlueprint, FName OldGraphName) const
{
	if (const UBlueprint* Blueprint = WeakBlueprint.Get())
	{
		if (!IsValidCPPIdentifier(InNewName.ToString()))
...
void FMGAHeaderViewFunctionListItem::OnRenameParameterTextCommitted(const FText& CommittedText, const ETextCommit::Type TextCommitType, const TWeakObjectPtr<UBlueprint> WeakBlueprint, FName OldGraphName, const FName OldParamName) const
{
	if (TextCommitType == ETextCommit::OnEnter)
	{
		if (UBlueprint* Blueprint = WeakBlueprint.Get())
...
void FMGAHeaderViewFunctionListItem::JumpToDefinition(const TWeakObjectPtr<UBlueprint> WeakBlueprint) const
{
	if (const UBlueprint* Blueprint = WeakBlueprint.Get())
	{
