...
#define LOCTEXT_NAMESPACE "FMGAHeaderViewClassListItem"

FMGAHeaderViewClassListItem::FMGAHeaderViewClassListItem(const TSharedPtr<FMGAAttributeSetWizardViewModel>& InViewModel)
{
	...
	if (const UBlueprint* Blueprint = InViewModel->GetSelectedBlueprint().Get())
	{
		...
		// i.e. UCLASS(Blueprintable, BlueprintType, Category=BlueprintCategory)
		{
			...
			// RichTextString += FString::Printf(TEXT("\n<%s>UCLASS</>(Blueprintable, BlueprintType%s)"), *MGA::HeaderViewSyntaxDecorators::MacroDecorator, *AdditionalSpecifiers);

			RawItemString += FString::Printf(TEXT("\nUCLASS(BlueprintType%s)"), *AdditionalSpecifiers);
			RichTextString += FString::Printf(TEXT("\n<%s>UCLASS</>(BlueprintType%s)"), *MGA::HeaderViewSyntaxDecorators::MacroDecorator, *AdditionalSpecifiers);
		}
		...
		// i.e. class ClassName : public ParentClass
		{
			...
			const FString& NameDecorator = bIsValidName ? MGA::HeaderViewSyntaxDecorators::TypenameDecorator : MGA::HeaderViewSyntaxDecorators::ErrorDecorator;
			...
			if (ClassLocation == GameProjectUtils::EClassLocation::Public && ModuleInfo.IsValid())
			{
				...
				MacroRichTextExportAPI = FString::Printf(TEXT(" <%s>%s_API</>"), *MGA::HeaderViewSyntaxDecorators::MacroDecorator, *ModuleInfo->ModuleName.ToUpper());
			}
...
FMGAHeaderViewListItemPtr FMGAHeaderViewClassListItem::Create(const TSharedPtr<FMGAAttributeSetWizardViewModel>& InViewModel)
{
	return MakeShared<FMGAHeaderViewClassListItem>(InViewModel);
}

void FMGAHeaderViewClassListItem::ExtendContextMenu(FMenuBuilder& InMenuBuilder, const TWeakObjectPtr<UObject> InAsset)
{
	if (!bIsValidName)
	{
		...
		if (UBlueprint* Blueprint = Cast<UBlueprint>(InAsset.Get()))
		{
			...
			InMenuBuilder.AddVerifiedEditableText(LOCTEXT("RenameBlueprint", "Rename Blueprint"),
				...
				FOnVerifyTextChanged::CreateSP(this, &FMGAHeaderViewClassListItem::OnVerifyRenameTextChanged, WeakBlueprint),
				FOnTextCommitted::CreateSP(this, &FMGAHeaderViewClassListItem::OnRenameTextCommitted, WeakBlueprint)
			);
		}
#endif
	}
...
FString FMGAHeaderViewClassListItem::GetConditionalUClassSpecifiers(const UBlueprint* Blueprint)
{
...
FString FMGAHeaderViewClassListItem::GetRenamedBlueprintPath(const UBlueprint* Blueprint, const FString& NewName) const
{
	check(Blueprint);
...
bool FMGAHeaderViewClassListItem::OnVerifyRenameTextChanged(const FText& InNewName, FText& OutErrorText, const TWeakObjectPtr<UBlueprint> InBlueprint) const
{
...
void FMGAHeaderViewClassListItem::OnRenameTextCommitted(const FText& CommittedText, const ETextCommit::Type TextCommitType, const TWeakObjectPtr<UBlueprint> InBlueprint) const
{
