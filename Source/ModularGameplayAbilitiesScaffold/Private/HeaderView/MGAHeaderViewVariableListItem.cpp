...
#define LOCTEXT_NAMESPACE "FMGAHeaderViewVariableListItem"

FMGAHeaderViewVariableListItem::FMGAHeaderViewVariableListItem(const FProperty& InVarProperty, const TSharedPtr<FMGAAttributeSetWizardViewModel>& InViewModel)
{
	// Format comment
	{
...
FMGAHeaderViewListItemPtr FMGAHeaderViewVariableListItem::Create(const FProperty& VarProperty, const TSharedPtr<FMGAAttributeSetWizardViewModel>& InViewModel)
{
	return MakeShareable(new FMGAHeaderViewVariableListItem(VarProperty, InViewModel));
}

void FMGAHeaderViewVariableListItem::ExtendContextMenu(FMenuBuilder& InMenuBuilder, const TWeakObjectPtr<UObject> InAsset)
{
	if (!IllegalName.IsNone())
	{
		...
		InMenuBuilder.AddVerifiedEditableText(LOCTEXT("RenameItem", "Rename Variable"),
			...
			FOnVerifyTextChanged::CreateSP(this, &FMGAHeaderViewVariableListItem::OnVerifyRenameTextChanged, InAsset),
			FOnTextCommitted::CreateSP(this, &FMGAHeaderViewVariableListItem::OnRenameTextCommitted, InAsset)
		);
#endif
	}
}

FBPVariableDescription* FMGAHeaderViewVariableListItem::GetBlueprintVariableDescription(const FName& InVarPropertyName, const TSharedPtr<FMGAAttributeSetWizardViewModel>& InViewModel)
{
...
void FMGAHeaderViewVariableListItem::FormatDelegateDeclaration(const FMulticastDelegateProperty& DelegateProp)
{
	if (const UFunction* SigFunction = DelegateProp.SignatureFunction)
	{
		...
		for (int32 ParamIdx = 0; ParamIdx < MacroParams.Num(); ++ParamIdx)
		{
			...
			MacroParams[ParamIdx] = FString::Printf(TEXT("<%s>%s</>"), bIsTypename ? *MGA::HeaderViewSyntaxDecorators::TypenameDecorator : *MGA::HeaderViewSyntaxDecorators::IdentifierDecorator, *MacroParams[ParamIdx]);
		}

		RichTextString += FString::Printf(TEXT("\n<%s>DECLARE_DYNAMIC_MULTICAST_DELEGATE%s</>(%s);"), *MGA::HeaderViewSyntaxDecorators::MacroDecorator, **NumParamsString, *FString::Join(MacroParams, TEXT(", ")));
	}
}

FString FMGAHeaderViewVariableListItem::GetConditionalUPropertySpecifiers(const FProperty& VarProperty) const
{
...
FString FMGAHeaderViewVariableListItem::GetOwningClassName(const FProperty& VarProperty)
{
...
bool FMGAHeaderViewVariableListItem::OnVerifyRenameTextChanged(const FText& InNewName, FText& OutErrorText, const TWeakObjectPtr<UObject> WeakAsset) const
{
...
void FMGAHeaderViewVariableListItem::OnRenameTextCommitted(const FText& CommittedText, const ETextCommit::Type TextCommitType, const TWeakObjectPtr<UObject> WeakAsset) const
{
