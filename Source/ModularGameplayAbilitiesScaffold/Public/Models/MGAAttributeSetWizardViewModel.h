...
#include "ViewModel/IMGASlateViewModel.h"
...
struct FMGARequiredModuleDependency
{
	FString ModuleName;
	FText Reason;

	FMGARequiredModuleDependency() = default;

	FMGARequiredModuleDependency(const FString& InModuleName, const FText& InReason)
		: ModuleName(InModuleName)
		, Reason(InReason)
	{
	}

	bool operator<(const FMGARequiredModuleDependency& InOther) const
	{
 ...
 */
class FMGAAttributeSetWizardViewModel : public IMGASlateViewModel
{
public:
	...
	DECLARE_VIEWMODEL_PROPERTY(EMGAPreviewCppType, CurrentPreviewValue);
	...
	DECLARE_VIEWMODEL_PROPERTY(TArray<FMGARequiredModuleDependency>, RequiredModuleDependencies);
	...
	FMGAAttributeSetWizardViewModel();

	/** Convenience constructor so you can construct from a FNewClassInfo */
	explicit FMGAAttributeSetWizardViewModel(const FNewClassInfo& InParentClassInfo);

	/** Convenience constructor so you can construct from a UClass */
	explicit FMGAAttributeSetWizardViewModel(const UClass* InClass);
...
private:
	...
	static TArray<FMGARequiredModuleDependency> ReservedModuleDependencies;
