...
enum class EMGAPreviewCppType : uint8;

class SMGAHeaderView : public SCompoundWidget, public FNotifyHook
{
public:
	SLATE_BEGIN_ARGS(SMGAHeaderView) {}
	SLATE_END_ARGS()

	void Construct(const FArguments& InArgs, const FAssetData& InAssetData, const TSharedPtr<FMGAAttributeSetWizardViewModel>& InViewModel);
	virtual ~SMGAHeaderView() override;
...
private:
	...
	TSharedPtr<FMGAAttributeSetWizardViewModel> ViewModel;
	...
	TSharedPtr<SListView<FMGAHeaderViewListItemPtr>> HeaderListView;

	/** List Items source */
	TArray<FMGAHeaderViewListItemPtr> HeaderListItems;

	/** Reference to the HeaderListView Widget */
	TSharedPtr<SListView<FMGAHeaderViewListItemPtr>> SourceListView;

	/** List Items source */
	TArray<FMGAHeaderViewListItemPtr> SourceListItems;
	...
	void HandlePreviewValueChanged(EMGAPreviewCppType InActiveTab) const;
	...
	TSharedRef<ITableRow> GenerateRowForItem(FMGAHeaderViewListItemPtr Item, const TSharedRef<STableViewBase>& OwnerTable) const;
	...
	TSharedPtr<SWidget> OnPreviewContextMenuOpening(EMGAPreviewCppType InPreviewType) const;

	/** Called when a List Item is double clicked */
	void OnItemDoubleClicked(FMGAHeaderViewListItemPtr Item) const;
