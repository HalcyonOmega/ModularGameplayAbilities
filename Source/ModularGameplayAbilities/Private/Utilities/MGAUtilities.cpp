...
FString FMGAUtilities::GetAttributeClassName(const UClass* Class)
{
	if (!Class)
	{
		return TEXT("None");
...
FString FMGAUtilities::GetAttributeClassName(FString ClassName)
{
...
void FMGAUtilities::GetAllAttributeProperties(TArray<FProperty*>& OutProperties, const FString InFilterMetaStr, const bool bInUseEditorOnlyData)
{
...
void FMGAUtilities::GetAllAttributeFromClass(const UClass* InClass, TArray<FProperty*>& OutProperties, const FString InFilterMetaStr, const bool bInUseEditorOnlyData)
{
	check(InClass);
...
bool FMGAUtilities::IsValidAttributeClass(const UClass* Class)
{
	if (!Class)
	{
		return false;
	...
	if (Class->ClassGeneratedBy)
	{
		...
		if (!Class->IsChildOf(UModularAttributeSetBase::StaticClass()))
		{
			return false;
		}
...
bool FMGAUtilities::IsValidCPPType(const FString& InCPPType)
{
...
void FMGAUtilities::SerializeAttributeSet(UAttributeSet* InAttributeSet, FArchive& InArchive)
{
...
void FMGAUtilities::SerializeAbilitySystemComponentAttributes(const UAbilitySystemComponent* InASC, FArchive& InArchive)
{
	if (!InArchive.IsSaveGame())
	{
		return;
	...
	for (UAttributeSet* AttributeSet : AttributeSets)
	{
		if (AttributeSet)
		{
			// Assuming AttributeSet is implementing serialize and invokes FMGAUtilities::SerializeAttributeSet:
...
/* Added Functions from GSC Class */
...
void FMGAUtilities::TryGrantAbility(UAbilitySystemComponent* InASC, const FMGAGameFeatureAbilityMapping& InAbilityMapping, FGameplayAbilitySpecHandle& OutAbilityHandle, FGameplayAbilitySpec& OutAbilitySpec)
{
	check(InASC);
	
	if (InAbilityMapping.AbilityType.IsNull())
	{
		UE_LOG(LogModularGameplayAbilities, Error, TEXT("Failed to Grant Ability \"%s\" because SoftClassPtr is null"), *InAbilityMapping.AbilityType.ToString())
		return;
	}
	...
	if (!ASC)
	{
		UE_LOG(LogModularGameplayAbilities, Error, TEXT("Failed to Grant Ability \"%s\" because ASC \"%s\" is not a UModularAbilitySystemComponent"), *GetNameSafe(AbilityType), *GetNameSafe(InASC))
		return;
	}
	...
	if (ASC->IsOwnerActorAuthoritative())
	{
		...
		{
			UE_LOG(LogModularGameplayAbilities, Verbose, TEXT("Authority, Grant Ability (%s)"), *AbilityType->GetName())
	...
	else
	{
		...
		UE_LOG(LogModularGameplayAbilities, Verbose, TEXT("AddActorAbilities: Not Authority, try to find ability handle from spec: %s"), *OutAbilityHandle.ToString())
	}
}

void FMGAUtilities::TryBindAbilityInput(UAbilitySystemComponent* InASC, const FMGAGameFeatureAbilityMapping& InAbilityMapping, const FGameplayAbilitySpecHandle& InAbilityHandle, const FGameplayAbilitySpec& InAbilitySpec, FDelegateHandle& OutOnGiveAbilityDelegateHandle, TArray<TSharedPtr<FComponentRequestHandle>>* OutComponentRequests)
{
	check(InASC);
	...
	if (!ASC)
	{
		UE_LOG(LogModularGameplayAbilities, Error, TEXT("Failed to bind Ability \"%s\" because ASC \"%s\" is not a UModularAbilitySystemComponent"), *InAbilityMapping.AbilityType.ToString(), *GetNameSafe(InASC))
		return;
	}
	...
	if (InputComponent)
	{
		UE_LOG(LogModularGameplayAbilities, Verbose, TEXT("Try to setup input binding for '%s': '%s' (%s)"), *InAbilityMapping.InputAction.ToString(), *InAbilityHandle.ToString(), *InAbilitySpec.Handle.ToString())
		...
		else
		{
			...
			OutOnGiveAbilityDelegateHandle = ASC->OnGiveAbilityDelegate.AddStatic(
				&FMGAUtilities::HandleOnGiveAbility,
...
void FMGAUtilities::TryGrantAttributes(UAbilitySystemComponent* InASC, const FMGAGameFeatureAttributeSetMapping& InAttributeSetMapping, UAttributeSet*& OutAttributeSet)
{
	check(InASC);
	...
	{
		UE_LOG(LogModularGameplayAbilities, Error, TEXT("Ability System Component owner actor is not valid"))
		return;
	}
	...
	if (!AttributeSetType)
	{
		UE_LOG(LogModularGameplayAbilities, Error, TEXT("AttributeSet class is invalid"))
		return;
	}
	...
	if (UAttributeSet* AttributeSet = GetAttributeSet(InASC, AttributeSetType))
	{
		...
		// UE_LOG(LogModularGameplayAbilities, Warning, TEXT("%s AttributeSet is already added to %s"), *AttributeSetType->GetName(), *OwnerActor->GetName())
		return;
	}
...
void FMGAUtilities::TryGrantGameplayEffect(UAbilitySystemComponent* InASC, const TSubclassOf<UGameplayEffect> InEffectType, const float InLevel, TArray<FActiveGameplayEffectHandle>& OutEffectHandles)
{
	check(InASC);
	...
	if (!InEffectType)
	{
		UE_LOG(LogModularGameplayAbilities, Warning, TEXT("Trying to apply an effect from an invalid class"))
		return;
	}
...
bool FMGAUtilities::TryGrantAbilitySet(UAbilitySystemComponent* InASC, const UMGAAbilitySet* InAbilitySet, FMGAAbilitySetHandle& OutAbilitySetHandle, TArray<TSharedPtr<FComponentRequestHandle>>* OutComponentRequests)
{
	check(InASC);
	...
	*/
	...
	for (const FMGAGameFeatureAbilityMapping& AbilityMapping : InAbilitySet->GrantedAbilities)
	{
		AbilitiesIndex++;
		
		if (AbilityMapping.AbilityType.IsNull())
		{
			UE_LOG(LogModularGameplayAbilities, Error, TEXT("GrantedAbilities AbilityType on ability set %s is not valid at Index %d"), *GetNameSafe(InAbilitySet), AbilitiesIndex  - 1);
			continue;
		}
		...
		if (AbilityMapping.InputTag.IsValid())
		{
	...
	for (const FMGAGameFeatureAttributeSetMapping& Attributes : InAbilitySet->GrantedAttributeSets)
	{
		AttributesIndex++;

		if (Attributes.AttributeSet.IsNull())
		{
			UE_LOG(LogModularGameplayAbilities, Error, TEXT("GrantedAttributes AttributeSet on ability set %s is not valid at Index %d"), *GetNameSafe(InAbilitySet), AttributesIndex - 1);
			continue;
		}
	...
	for (const FMGAGameFeatureEffectMapping& Effect : InAbilitySet->GrantedEffects)
	{
		EffectsIndex++;
		
		if (Effect.EffectType.IsNull())
		{
			UE_LOG(LogModularGameplayAbilities, Error, TEXT("GrantedEffects EffectType on ability set %s is not valid at Index %d"), *GetNameSafe(InAbilitySet), EffectsIndex - 1);
			continue;
		}
...
UAttributeSet* FMGAUtilities::GetAttributeSet(const UAbilitySystemComponent* InASC, const TSubclassOf<UAttributeSet> InAttributeSet)
{
	check(InASC);
...
bool FMGAUtilities::HasGameplayEffectApplied(const UAbilitySystemComponent* InASC, const TSubclassOf<UGameplayEffect>& InEffectType, TArray<FActiveGameplayEffectHandle>& OutEffectHandles)
{
	check(InASC);
...
bool FMGAUtilities::IsAbilityGranted(const UAbilitySystemComponent* InASC, TSubclassOf<UGameplayAbility> InAbility, const int32 InLevel)
{
	check(InASC);
...
UActorComponent* FMGAUtilities::FindOrAddComponentForActor(UClass* InComponentType, const AActor* InActor, TArray<TSharedPtr<FComponentRequestHandle>>& OutComponentRequests)
{
	check(InActor);
...
void FMGAUtilities::AddLooseGameplayTagsUnique(UAbilitySystemComponent* InASC, const FGameplayTagContainer& InTags, const bool bReplicated)
{
	check(InASC);
...
void FMGAUtilities::RemoveLooseGameplayTagsUnique(UAbilitySystemComponent* InASC, const FGameplayTagContainer& InTags, const bool bReplicated)
{
	check(InASC);
...
/*void FMGAUtilities::HandleOnGiveAbility(FGameplayAbilitySpec& InAbilitySpec, TWeakObjectPtr<UGSCAbilityInputBindingComponent> InInputComponent, TWeakObjectPtr<UInputAction> InInputAction, const EModularAbilityActivationPolicy InTriggerEvent, FGameplayAbilitySpec InNewAbilitySpec)
{
	/* @TODO: Fix or cleanup
	UE_LOG(LogModularGameplayAbilities, 
		Verbose,
