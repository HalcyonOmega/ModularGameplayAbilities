...
struct FMGAAttributeSetExecutionData
{
	...
	UPROPERTY(BlueprintReadOnly, Category = "ModularGameplayAbilities|Attribute")
...
/**
 * Place in a ModularAttributeSet to create an attribute that can be accessed using FGameplayAttribute.
 *
 ...
 */
UCLASS(meta=(DisplayName="Modular Attribute Set Base")) /* @TODO: Not Including Abstract specifier to test functionality - cleanup or readd. */
class MODULARGAMEPLAYABILITIES_API UModularAttributeSetBase : public UAttributeSet
{
	GENERATED_BODY()
	REPLICATED_BASE_CLASS(UModularAttributeSetBase)

public:
	// Sets default values for this AttributeSet attributes
	explicit UModularAttributeSetBase(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
...
public:
	...
	bool K2_PreGameplayEffectExecute(const FGameplayAttribute& InAttribute, const FMGAAttributeSetExecutionData& InData);
	...
	void K2_PostGameplayEffectExecute(const FGameplayAttribute& Attribute, const FMGAAttributeSetExecutionData& Data);
	...
	/** Returns true if the variable associated with Property is of type FMGAClampedAttributeData or one of its subclasses */
...
protected:
	...
	/**
	 * Called during construction from InitFromMetaDataTable(), this ensures FMGAClampedAttributeData clamps
	...
	/** Returns whether given Attribute is defined using a FMGAClampedAttributeData property, and if it has valid clamping values */
	bool IsValidClampedProperty(const FGameplayAttribute& Attribute);

	/** Returns the new value for an attribute after clamping via FMGAClampedAttributeData defaults Min / Max values */
